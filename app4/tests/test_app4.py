...
class TestPowerCall(TestBase):
    def test_app4(self):
        ...
        expected = "Muzzle Velocity=110 Elevation=45 Distance Travelled=1234 : Overshot - LOSER!"
        self.assertEqual(response.data.decode('utf-8'), expected)
        
        # Undershot
        response = self.client.post(url_for('fire'), data='45,89')
        self.assert200(response)
        expected = "Muzzle Velocity=89 Elevation=45 Distance Travelled=808 : Undershot - LOSER!"
        self.assertEqual(response.data.decode('utf-8'), expected)        
